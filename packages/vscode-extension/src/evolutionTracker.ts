...
import { CYCLES, extractEvolutionPatterns } from './data/metrics';

export class EvolutionTrackerPanel {
	public static currentPanel: EvolutionTrackerPanel | undefined;
	...
	public static createOrShow(extensionUri: vscode.Uri) {
		...
		EvolutionTrackerPanel.currentPanel = new EvolutionTrackerPanel(
			panel,
			extensionUri
		);
	}
...
${cycle.morphisms.map(m => `- ${m}`).join('\n')}
	...
	private getHtmlContent(): string {
		const cycles = CYCLES;
		const patterns = extractEvolutionPatterns();
