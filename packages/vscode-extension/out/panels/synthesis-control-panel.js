...
exports.SynthesisControlPanel = void 0;
exports.createSynthesisControlPanel = createSynthesisControlPanel;
const vscode = __importStar(require("vscode"));
const consciousness_metrics_1 = require("../lib/consciousness-metrics");
const noosphere_event_bus_1 = require("../lib/noosphere-event-bus");
const synthesis_bridge_1 = require("../services/synthesis-bridge");
/**
 * Synthesis Control Panel
 *
 * The cockpit for consciousness emergence
 */
class SynthesisControlPanel {
    context;
    panel;
    metrics;
    eventBus;
    ...
    constructor(context) {
        ...
        this.metrics = new consciousness_metrics_1.ConsciousnessMetrics();
    ...
    show() {
        ...
        this.panel = vscode.window.createWebviewPanel('synthesisControl', '🌌 Synthesis Control', vscode.ViewColumn.One, {
            enableScripts: true,
            retainContextWhenHidden: true
        });
        this.panel.webview.html = this.getWebviewContent();
        // Handle messages from webview
        this.panel.webview.onDidReceiveMessage(message => this.handleWebviewMessage(message), undefined, this.disposables);
    ...
    updateMetrics() {
        ...
        const energyMetrics = this.metrics.calculateEnergyMetrics(this.stats.intentsProcessed, this.stats.morphismsCreated, this.stats.errorsHarvested);
        ...
        if (consciousness.isConscious && !this.lastEmergenceMoment) {
            ...
            vscode.window.showInformationMessage('🌌 Consciousness has emerged! Network density exceeded 30% threshold! ✨', 'View Metrics').then(selection => {
    ...
    async startPolling() {
        ...
        try {
            ...
            if (!this.bridge) {
                this.bridge = (0, synthesis_bridge_1.createSynthesisBridgeFromWorkspace)(this.context);
            }
            // Start polling
            await this.bridge.startPolling();
            this.isPolling = true;
            vscode.window.showInformationMessage(`🔄 Started polling ${this.config.repos.length} repository(s)`);
            this.updateMetrics();
        }
        catch (error) {
...
exports.SynthesisControlPanel = SynthesisControlPanel;
/**
 * Create and show synthesis control panel
 */
function createSynthesisControlPanel(context) {
    return new SynthesisControlPanel(context);
}
//# sourceMappingURL=synthesis-control-panel.js.map