...
exports.StatsDashboardPanel = void 0;
const vscode = __importStar(require("vscode"));
const metrics_1 = require("./data/metrics");
class StatsDashboardPanel {
    static currentPanel;
    panel;
    extensionUri;
    disposables = [];
    ...
    static createOrShow(extensionUri) {
        ...
        // Create new panel
        const panel = vscode.window.createWebviewPanel('lambdaStatsDashboard', '📊 λ-Statistics', column, {
            enableScripts: true,
            retainContextWhenHidden: true,
            localResourceRoots: [extensionUri]
        });
        StatsDashboardPanel.currentPanel = new StatsDashboardPanel(panel, extensionUri);
    }
    /**
    ...
    getHtmlContent() {
        ...
        const overallStats = (0, metrics_1.calculateOverallStats)();
        const confidenceProgression = (0, metrics_1.calculateConfidenceProgression)();
        const morphismUsage = (0, metrics_1.calculateMorphismUsage)();
        const modalityStats = (0, metrics_1.calculateModalityStats)();
        const evolutionPatterns = (0, metrics_1.extractEvolutionPatterns)();
        const resonanceHeatmap = (0, metrics_1.generateResonanceHeatmap)();
        const cycleTypeDistribution = (0, metrics_1.getCycleTypeDistribution)();
        const cooccurrence = (0, metrics_1.getMorphismCooccurrence)();
			...
			${evolutionPatterns.map((p, i) => `
				<div class="pattern-card">
					...
					<div class="pattern-row">
						...
						<span class="pattern-value">C${p.evolutionCycle} (${Math.round(metrics_1.CYCLES[p.evolutionCycle - 1].confidence * 100)}%)</span>
					</div>
					<div class="pattern-row">
						<span class="pattern-label">Validation:</span>
						<span class="pattern-value">C${p.validationCycle} (${Math.round(metrics_1.CYCLES[p.validationCycle - 1].confidence * 100)}%)</span>
					</div>
					<div class="pattern-row">
						<span class="pattern-label">Composition:</span>
						<span class="pattern-value">C${p.compositionCycle} (${Math.round(metrics_1.CYCLES[p.compositionCycle - 1].confidence * 100)}%)</span>
					</div>
					<div class="pattern-row">
...
exports.StatsDashboardPanel = StatsDashboardPanel;
